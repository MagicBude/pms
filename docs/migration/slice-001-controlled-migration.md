...
真实 10 行切片已由业务使用者复核并授权按工程化口径处理；`business_confirmed` 包在隔离库完成
29 项逐项一致对账，`AC-S001-043` 已签收。同项目 3 条运费需求被确认属于非物料服务/物流费用，
不进入 BOM；详见[真实项目切片映射与复核](real-slice-review.md)。
