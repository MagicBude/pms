...
浏览器访问 `http://127.0.0.1:8000/`，未登录时应转到登录页，登录后进入 PMS 工作台。首次启动
会自动创建被 Git 忽略的本机数据目录、`attachments/` 私有子目录和 SQLite 文件。开发服务器
只用于开发；本机日常使用下文的正式启动器。
...
本机数据备份、离线校验和空目录恢复命令见
[本机备份与恢复](../deployment/local-backup-restore.md)。执行这些操作前应停止 PMS 服务，恢复后
先使用新的 `PMS_DATA_DIR` 完成检查和 ready 验证，不要直接覆盖原数据目录。
